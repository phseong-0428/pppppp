...
# 공 크기에 따른 최초 스피드 (공이 떠오르는 높이 조정)
ball_speed_y = [-18, -15, -12, -9] # 나중에 조절//index 0, 1, 2, 3 에 해당하는 값
...
balls.append({
    "pos_x" : 50, #공 x 좌표
    "pos_y" : 50, #공 y 좌표
    "img_idx" : 0, # 공의 이미지 인덱스 (한 번 맞으면 숫자 늘어나면서 공 크기 작아짐)
    "to_x" : 3, #x축 이동방향, -3이면 왼쪽 3이면 오른쪽
    "to_y" : -6, #6이면 아래쪽, -6이면 위쪽
...
while running:
    ...
    # 공 위치 정의
    for ball_idx, ball_val in enumerate(balls): # balls list에서 
    ...
    for ball_idx, ball_val in enumerate(balls): # balls list에서 
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:
                    ...
                    balls.append({
                        ...
                        "img_idx" : ball_img_idx + 1, # 공의 이미지 인덱스 (한 번 맞으면 숫자 늘어나면서 공 크기 작아짐)
                        "to_x" : -3, #x축 이동방향, -3이면 왼쪽 3이면 오른쪽
                        "to_y" : -6, #6이면 아래쪽, -6이면 위쪽
        ...
        break   # 안쪽 for 에서 break 만나면 여기로 도착 -> for 문 탈출
